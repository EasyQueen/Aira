...
const MODEL_ICON_CONFIG: Record<ModelKey, { path: string; viewBox: string }> = {
  grok: {
    path: 'M25.753 5.485c-.496 0-.967.247-1.25.663L14.73 19.824l-3.87-5.59a1.53 1.53 0 0 0-1.251-.664H2.43c-.63 0-1.19.38-1.42 0.963-.23.582-.08 1.246.38 1.68l9.742 9.097a1.53 1.53 0 0 0 2.091 0l19.349-18.067a1.53 1.53 0 0 0 .428-1.55 1.53 1.53 0 0 0-1.447-1.208h-6.2zM8.247 27.515c.496 0 .967-.247 1.25-.663l9.773-13.676 3.87 5.59c.283.416.754.664 1.251.664h7.179c.63 0 1.19-.38 1.42-.963.23-.582.08-1.246-.38-1.68l-9.742-9.097a1.53 1.53 0 0 0-2.091 0L1.428 25.757a1.53 1.53 0 0 0-.428 1.55 1.53 1.53 0 0 0 1.447 1.208h6.2z',
    viewBox: '0 0 34 33',
  },
  codex: {
...
function platformMarkSvg(key: ModelKey): string {
  const icon = MODEL_ICON_CONFIG[key]
  return `<span class="platform-mark ${key}" aria-hidden="true"><svg viewBox="${icon.viewBox}" focusable="false"><path d="${icon.path}" fill="currentColor"/></svg></span>`
}
...
function orderedWindows(account: TrayAccount): TrayWindow[] {
  ...
  if (modelKey(account.platform) === 'grok') return windows
  return [
